...
def register(cli):  # pragma: no cover - exercised via llm runtime
    ...
    @click.option(
        ...
        help="Output CSV path. Required unless --in-place is provided.",
    )
    @click.option("--in-place", is_flag=True, help="Overwrite the input CSV in place")
    @click.option(
        "-c",
        "--column",
        "target_column",
    ...
    @click.option(
        ...
        type=click.Path(dir_okay=False, path_type=Path),
        required=True,
        help="Destination CSV for reduced output",
    )
    @click.option(
        "-c",
        "--column",
    ...
    def reduce_command(
        ...
        write_csv(output, results, ["group", result_column])
